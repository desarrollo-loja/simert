...
export class SimertService {
        ...
        try {
            const fractions = await this.fractionRepository
                ...
                .andWhere('f.status < :status', {
                    status: StatusFraction.FINISHED,
                })
                .getMany();
    ...
    async incrementTime(
        ...
        const fractionOld = await this.fractionRepository
            ...
            .innerJoin('f.slot', 'slot')
            .where('f.id = :fractionId', { fractionId })
            .andWhere('f.status < :status1', {
                status1: StatusFraction.SANCTIONED,
            })
            ...
            switch (fractionByPlate.status.id) {
                ...
                case StatusFraction.EXCEEDED_TIME:
                    return {
                        ...
                        message:
                            'La placa tiene una fracción con tiempo excedido',
                    };
        ...
        try {
            const [slot, checkboxes, dateRow] = await Promise.all([
                this.slotRepository
                    ...
                    .andWhere('s.isActivated = :isActivated', {
                        isActivated: true,
                    })
                    ...
                    .andWhere('zone.lt != :zero AND zone.lg != :zero', {
                        zero: 0,
                    })
