...
export class CheckboxService implements OnModuleInit {
    ...
    async getTransactions(
        ...
        try {
            ...
            const checkboxs = await this.checkboxRepository.query(
                query,
                params,
            );
        ...
        try {
            const [cards, checkboxes] = await Promise.all([
                this.cardRepository
                    ...
                    .where('cd.isActivated = :isActivated', {
                        isActivated: true,
                    })
                    .getMany(),
    ...
    async buyCheckboxs(idDevice: string, createCheckboxDto: CreateCheckboxDto) {
        ...
        try {
            ...
            try {
                ...
                this.logger.error(
                    `call buyCheckboxs error.message ${error.message}`,
                );
            } finally {
    ...
    async _saveResponsePay(
        ...
        if (statusPayment === StatusPayment.PAID) {
            try {
                ...
                if (
                    emisionResult &&
                    emisionResult.errorCode !== ErrorCode.NONE
                ) {
                    this.logger.error(
                    ...
                    if (emisionResult.dataEmision)
                        checkbox.onResponseExternal.push(
                            emisionResult.dataEmision,
                        );
                } else {
                    ...
                    if (
                        depositResult &&
                        depositResult.errorCode !== ErrorCode.NONE
                    ) {
                        if (depositResult.dataDeposit)
                            checkbox.onResponseExternal.push(
                                depositResult.dataDeposit,
                            );
                        checkbox.statusIncident = IncidentStatus.SUPPLIED;
    ...
    private _scheduleUnconfirmedCheckboxReversal(
        ...
        setTimeout(async () => {
            ...
            debitAmounDto.transactionFootbridge = {
                id: this._resolveTransactionFootbridge(
                    checkbox.typePaymentMethod,
                ),
            };
