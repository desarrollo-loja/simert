...
export class PublicService implements IPublicService {
    ...
    async findAllZones(filter: PublicFilterDto): Promise<ZoneListResponse> {
        ...
        if (filter.search) {
            qb.andWhere('z.name ILIKE :search', {
    ...
    async findAllSectors(filter: PublicFilterDto): Promise<SectorListResponse> {
        ...
        if (filter.search) {
            qb.andWhere('b.name ILIKE :search', {
                search: `%${filter.search}%`,
            });
        }
    ...
    async findOccupancySummary(): Promise<OccupancySummaryResponse> {
        ...
        ORDER BY z.name ASC`,
                ...
                this.blockRepository
                    ...
                    .innerJoin('b.zone', 'z')
                    .where('b.isActivated = :active', { active: true })
                    .andWhere('z.isActivated = :zoneActive', {
                        zoneActive: true,
                    })
                    .getCount(),
            ]);
        ...
        const occupancyRate =
            totalSlots > 0
                ? Math.round(
                      (occupied / (totalSlots - (global.outOfService ?? 0))) *
                          10000,
                  ) / 100
                : 0;
        ...
        const zones = (zoneStats ?? []).map((z: any) => {
            ...
            return {
                ...
                occupancyRate:
                    zTotal > 0
                        ? Math.round((zOccupied / zTotal) * 10000) / 100
                        : 0,
            };
        });
