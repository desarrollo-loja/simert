...
export class GimService {
    ...
    private _authJsonHeaders(): Record<string, string> {
        ...
        // No token means the GIM 2 login performed by CommonGimService failed, so
        // the outbound call is already doomed; record it, since the resulting 401
        // would otherwise be audited without its actual cause.
        if (!token) {
            this._logGimFailure({
                method: 'getToken',
                endpoint: `${this.gimBaseUrlLogin}/realms/${this.gim2RealmMunicipio}/protocol/openid-connect/token`,
                errorCode: ErrorCode.UNAUTHORIZED,
                message:
                    'No hay token GIM 2 disponible para consumir el recurso',
            });
        }
    ...
    /**
     * Writes a GIM failure to the `logsgim` collection. Single entry point for
     * every audit write in this service; fire-and-forget, so it never alters the
     * caller flow.
     *
     * @param fields Failure details recorded in the audit document.
     * @param fields.method Operation or GIM endpoint path that failed.
     * @param fields.endpoint GIM endpoint URL invoked.
     * @param fields.httpStatus HTTP status of the GIM response, when there is one.
     * @param fields.errorCode Mapped application error code.
     * @param fields.message Human-readable failure reason.
     * @param fields.response Raw GIM response body.
     * @param fields.exception Exception summary, for thrown errors.
     */
    private _logGimFailure(fields: {
        method: string;
        endpoint: string;
        httpStatus?: number;
        errorCode?: number;
    ...
    /**
     * Records a GIM call that threw: transport failure, timeout, 4xx or 5xx.
     *
     * Every error is audited, not only the ones {@link _gimServerErrorOrNull}
     * classifies as "GIM unavailable" — that classifier is used solely to pick
     * the error code and message, so a rejected token (401), a disabled resource
     * (403/404) or a rejected payload (400) stay queryable instead of being
     * visible only in stdout.
     *
     ...
     */
    private _logGimError(method: string, endpoint: string, error: any): void {
        ...
        this._logGimFailure({
            ...
            errorCode: serverError?.errorCode ?? ErrorCode.HTTP_ERROR_REINTENT,
            message:
                serverError?.message ??
                error?.response?.data?.message ??
                error?.message ??
                'Error al consumir el recurso del GIM',
    ...
    /**
     * Records a GIM call that answered HTTP 200 with a negative payload
     * (`ok: false` or a non-200 `code`), keeping the raw body so the rejection
     * reason (code / message / validationErrors) is available for diagnosis.
     *
     * Complements {@link _logGimError}, which covers the thrown cases. Callers
     * only invoke this for genuine rejections: an empty result that the caller
     * treats as normal control flow (taxpayer not registered yet, debt not issued
     * yet) is not a failure and is deliberately left unlogged.
     *
     * @param method Operation or GIM endpoint path that was rejected.
     * @param endpoint GIM endpoint URL invoked.
     * @param response Raw GIM response body.
     * @param message Human-readable rejection reason surfaced to the caller.
     */
    private _logGimRejection(
        method: string,
        endpoint: string,
        response: any,
        message?: string,
     ...
     * `createNewNaturalPerson`, shared by the two creation flows.
     *
    ...
    private _logGimCreateFailure(
        ...
        this._logGimFailure({
            method,
            endpoint: this._externalApiUrl('createNewNaturalPerson'),
            ...fields,
        });
    }
    ...
    async issueIncidentGim(
        ...
        try {
            ...
            this._logGimError(
                'issueIncidentGim',
                this._externalApiUrl('emitSimertSanction'),
                error,
            );
            return {
                errorCode: ErrorCode.NOT_FOUND,
    ...
    > {
        try {
            ...
            // The thrown failure is already persisted in logsgim by
            // _postToExternalApi, which audits every error it catches.
    ...
    async verifateIncidentGim(
        ...
        const url = this._externalApiUrl('verifateIncidentSimert');
        ...
        try {
            ...
            this._logGimRejection('verifateIncidentGim', url, data);
            ...
            this._logGimError('verifateIncidentGim', url, error);
    ...
    async emitInfractionGim(createGimDto: CreateGimDto): Promise<{
        ...
        try {
            ...
            const url = this._externalApiUrl('emitSimertSanction');
            ...
            this._logGimRejection('emitInfractionGim', url, data);
    ...
    async findBondByNumber(
        ...
        const url = this._externalApiUrl('findBondByNumber');
        ...
        try {
            ...
            if (
                ...
                // A bond that simply does not exist is normal control flow; only an
                // explicit GIM rejection is worth auditing.
                if (data && data.ok === false) {
                    this._logGimRejection('findBondByNumber', url, data);
                }
                return {
                    errorCode: ErrorCode.NOT_FOUND,
    ...
    private _normalizePaidObligationsPage(
        ...
        const items = (candidates.find((candidate) =>
            Array.isArray(candidate),
        ) ?? []) as PaidObligationGim[];

        const total = Number(
            data?.totalElements ?? data?.total ?? items.length,
        );
    ...
    private _paidObligationAmount(paidObligation: PaidObligationGim): number {
        const raw =
            paidObligation?.total ??
            paidObligation?.amount ??
            paidObligation?.value;
     ...
     *
     * Thin wrapper over {@link _logGimFailure} that pins the method and the
     * endpoint, which lives on its own host (`GIM_BASE_URL_PAID_OBLIGATIONS`).
     * Covers the business rejections (HTTP 200 with `ok: false` / a non-200 code)
     * that {@link _logGimError} cannot see, since those never throw.
     *
     * @param fields Failure details captured from the GIM response.
     * @param fields.httpStatus HTTP status of the GIM response (200 for business rejections).
     * @param fields.errorCode Mapped application error code.
    ...
    async findPaidObligations(paidObligationsDto: PaidObligationsDto): Promise<{
        ...
        const rangeError = this._validatePaidObligationsRange(
            startDate,
            endDate,
        );
        if (rangeError) {
        ...
        try {
            ...
            const hasEnvelope =
                data?.ok !== undefined || data?.code !== undefined;
    ...
    public async _validateStatusSistemWithGim(
        ...
        try {
            ...
            switch (status) {
                ...
                default: {
                    // An obligation status this service does not know about means GIM
                    // changed its catalog; audit it so the unmapped value is traceable.
                    const unmappedMessage = `Error al validar el estado de la deuda en el GIM (estado no mapeado: ${status})`;
                    this._logGimFailure({
                        method: '_validateStatusSistemWithGim',
                        endpoint: this._externalApiUrl(
                            'findObligationsByCitation',
                        ),
                        errorCode: ErrorCode.NOT_FOUND,
