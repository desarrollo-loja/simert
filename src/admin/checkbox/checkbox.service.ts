...
export class CheckboxService {
    ...
    async findAll(filterDto: FilterDto) {
        ...
        `;
                ...
                const checkbox = await this.checkboxRepository.query(
                    query,
                    parameters,
                );
    ...
    async findAllByTransactionId(filterDto: FilterDto) {
        ...
        try {
            ...
            if (dateFrom && dateTo) {
                ...
                return await this._runFindByTransactionIdQuery(
                    fromSource,
                    filterDto,
                );
            }
        ...
        try {
         ...
         WHERE "statusPayment" = $1
           AND "statusIncident" IS NULL`,
            ...
            this.logger.error(
                `findPaidWithoutIncident error: ${error.message}`,
            );
        ...
        try {
         ...
         WHERE "statusPayment" = $1
           AND "statusIncident" IN (${placeholders})`,
            ...
            this.logger.error(
                `findPaidWithPendingIncident error: ${error.message}`,
            );
            return { errorCode: ErrorCode.HTTP_ERROR_REINTENT, data: [] };
        }
    }
