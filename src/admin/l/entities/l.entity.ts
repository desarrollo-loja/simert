...
@Entity('l')
@Index('idxLUserIdTakenTimestampLatitudeLongitude', ['userId', 'taken', 'timestamp', 'latitude', 'longitude'])
export class L {

    @PrimaryColumn({ primaryKeyConstraintName: 'pkLId', comment: 'User identifier, used as primary key for real-time location tracking' })
    @IsNumber()
    userId: number;
