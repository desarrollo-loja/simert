...
export class BlockOperatorService {
    ...
    async create(
        userId: number,
        createBlockOperatorDto: CreateBlockOperatorDto,
    ) {
        try {
    ...
    async findAll(filterDto: FilterDto) {
        ...
        try {
            let blockOperatorsQuery = this.blockOperatorRepository
                ...
                .addSelect(
                    `TO_CHAR(bo."to", 'YYYY-MM-DD"T"HH24:MI:SS.MS')`,
                    'bo_to',
                )
                .addSelect(
    ...
    async findAllActiveByUserId(filterDto: FilterDto) {
        ...
        try {
            const result = await this.blockOperatorRepository
                ...
                .andWhere('bo.isActivated = :isActivated', {
                    isActivated: true,
                })
                .andWhere('bo.isInitialized = :isInitialized', {
                    isInitialized,
                })
    ...
    async findUniqueUsers(dto: FindUniqueUsersBlockOperatorDto) {
        ...
        try {
            const query = this.blockOperatorRepository
                ...
                .andWhere('bo.isFinalized = :isFinalized', {
                    isFinalized: false,
                });
