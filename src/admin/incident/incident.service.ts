...
export class IncidentService {
    ...
    async findAllClient(filterDto: IncidentFilterDto) {
      ...
      `;
            ...
            const clients = await this.incidentRepository.query(
                query,
                parameters,
            );
    ...
    private async _resolveIncidentTable(
        ...
        if (year && month && this._isValidYearMonth(year, month)) {
            ...
            const tableExistsIncident =
                await this._tableExists(tableNameIncidentAux);
    ...
    private async _resolveSanctionTables(filterDto: {
        ...
        if (year && month && this._isValidYearMonth(year, month)) {
            ...
            const tableExistsIncident =
                await this._tableExists(tableNameIncidentAux);
            const tableExistsFraction =
                await this._tableExists(tableNameFractionAux);
    ...
    async update(
        ...
        try {
            ...
            if (
                dtoYear &&
                dtoMonth &&
                this._isValidYearMonth(dtoYear, dtoMonth)
            ) {
    ...
    private _convertRangeToTimeZone(
        ...
        try {
            ...
            this.logger.error(
                `_convertRangeToTimeZone error: ${error?.message}`,
            );
            return { start: '', end: '' };
        }
    }
    ...
    private _buildConditionsAndParametersPg(
        ...
        if (blockOperatorId) {
            conditions.push(
                `i."blockOperatorId" = ${addParam(blockOperatorId)}`,
            );
        }

        if (incidentCategory) {
            conditions.push(
                `i."incidentCategory" = ${addParam(incidentCategory)}`,
            );
        }
  ...
  `;
    ...
    async findAllTotalVehicleClientTime(filterDto: IncidentFilterDto) {
        try {
            ...
            if (typeFractionId) {
                parameters.push(typeFractionId);
                conditions.push(
                    `fraction."typeFraction" = $${parameters.length}`,
                );
            }
        ...
        try {
            ...
            for (const incident of incidents) {
                ...
                if (incidentsSupplied.length === 0) {
                    messageInfo =
                        'No se encontraron incidencias para sincronizar';
                }
            }
    ...
    async advanceNextProcess(
        ...
        if (
            ...
            incidentDto.internalState ===
            InternalStateIncident.REVENUE_DEPARTMENT
        ) {
            internalState = InternalStateIncident.REVENUE_DEPARTMENT;
        }
