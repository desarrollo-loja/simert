...
export class Schedule {
    ...
    @Column('boolean', {
        default: true,
        comment: 'Whether this schedule rule is currently active',
    })
    isActivated: boolean;

    @Column('smallint', {
        comment: 'Day of the week the schedule starts (0=Sunday, 6=Saturday)',
    })
    dayOfWeekInit: number;

    @Column('smallint', {
        comment: 'Day of the week the schedule ends (0=Sunday, 6=Saturday)',
    })
    dayOfWeekEnd: number;

    @Column('time', {
        comment: 'Time of day when the paid parking period opens (HH:mm:ss)',
    })
    openingTime: string;

    @Column('time', {
        comment: 'Time of day when the paid parking period closes (HH:mm:ss)',
    })
    closingTime: string;
    ...
    @ManyToOne(() => Block, (block) => block.schedules, { onDelete: 'CASCADE' })
    @JoinColumn({
        name: 'blockId',
        foreignKeyConstraintName: 'fkScheduleBlock',
    })
    block: Block;
}
