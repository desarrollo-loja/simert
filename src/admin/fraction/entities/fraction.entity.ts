...
export class Fraction {
    ...
    @PrimaryGeneratedColumn('increment', {
        primaryKeyConstraintName: 'pkFractionId',
    })
    @IsNumber()
    id: number;
    ...
    @ApiProperty({
        example: 1,
        description: 'Service section city identifier for internal routing',
    })
    @Column('int', {
        comment: 'Service section city identifier for internal routing',
    })
    ...
    @ApiProperty({
        example: 123,
        description:
            'User identifier who owns this parking session (operator or client)',
    })
    @Column('int', {
        comment:
            'User identifier who owns this parking session (operator or client)',
    ...
    @ApiProperty({
        example: 'b6fabf7a-9a4e-4c9e-83a2-8ef6b39c8b21',
        maxLength: 36,
        description:
            'Unique transaction ID sent by the API consumer for idempotency checks',
    })
    @Column('varchar', {
        length: 36,
        comment:
    ...
    @ApiProperty({
        enum: TypeFraction,
        description: 'Fraction type (TypeFraction)',
    })
    @Column({
        type: 'int',
        comment:
            'Fraction type: references TypeFraction enum (PHYSICAL, VIRTUAL, etc.)',
    })
    ...
    @ApiProperty({
        example: '00:15:00',
        description:
            'Duration of one parking fraction for the block where this session was registered (HH:mm:ss)',
    })
    @Column('time', {
        comment:
            'Duration of one parking fraction for the block where this session was registered (HH:mm:ss)',
    ...
    @ApiProperty({
        example: '00:00:00',
        description:
            'Accumulated time from previous increments before this fraction (HH:mm:ss)',
    })
    @Column('time', {
        default: '00:00:00',
        comment:
    ...
    @ApiProperty({
        example: '00:30:00',
        description:
            'Total parking time for this fraction, auto-calculated on insert (HH:mm:ss)',
    })
    @Column('time', {
        comment:
            'Total parking time for this fraction, auto-calculated on insert from checkboxes * timeByBlock (HH:mm:ss)',
    })
    time: string;
    ...
    @ApiProperty({
        example: 2,
        description:
            'Number of parking fractions purchased by the user for this session',
    })
    @Column('int', {
        comment:
            'Number of parking fractions purchased by the user for this session',
    ...
    @ApiPropertyOptional({
        ...
        description: 'Physical or virtual card number used for this session',
    })
    ...
    @Column('varchar', {
        ...
        nullable: true,
        comment:
            'Physical or virtual card number used for this session (null for plate-based sessions)',
    })
    card: string;

    @ApiProperty({
        example: 'My car',
        description: 'Alias or nickname assigned to the vehicle by the user',
    })
    @Column('varchar', {
        length: 25,
        default: '',
        comment: 'Alias or nickname assigned to the vehicle by the user',
    })
    alias: string;

    @ApiProperty({
        example: 'red',
        description: 'Vehicle color tint identifier used for display purposes',
    })
    @Column('varchar', {
        length: 25,
        comment: 'Vehicle color tint identifier used for display purposes',
    })
    tint: string;

    @ApiProperty({
        example: 'https://cdn.example.com/img.png',
        description: 'URL of the vehicle image uploaded by the user',
    })
    @Column('varchar', {
        length: 255,
        default: '',
        comment: 'URL of the vehicle image uploaded by the user',
    })
    image: string;
    ...
    @ApiProperty({
        ...
        format: 'date-time',
        description:
            'Operation datetime (adjusted to UTC when sent by operator)',
    })
    ...
    @ApiProperty({
        type: String,
        format: 'date-time',
        description: 'Calculated departure datetime based on registerAt + time',
    })
    @Index('idxFractionDepartureDate')
    @Column({
        type: 'timestamp',
        comment:
            'Calculated departure datetime based on registerAt + time. Auto-computed on insert',
    })
    departureDate: Date;
    ...
    @ApiPropertyOptional({
        description:
            'Raw device/client metadata captured at registration time. Must never be exposed via web endpoints',
    })
    @Column({
        type: 'json',
        comment:
    ...
    @ManyToOne(() => Slot, (slot) => slot.fractions, {
        onDelete: 'RESTRICT',
        nullable: false,
        eager: false,
    })
    ...
    @ManyToOne(() => Block, (block) => block.fractions, {
        onDelete: 'RESTRICT',
        nullable: false,
        eager: false,
    })
    ...
    @ManyToOne(() => Status, (status) => status.fractions, { nullable: false })
    @JoinColumn({
        name: 'statusId',
        foreignKeyConstraintName: 'fkFractionStatus',
    })
    status: Status;

    @ApiProperty({
        example: '2024-05-10 10:30:00',
        description:
            'Business-level registration datetime of the session, set by the application',
    })
    @Column({
        type: 'timestamp',
        nullable: false,
    ...
    @ApiPropertyOptional({
        type: () => FractionStatus,
        isArray: true,
        description: 'Historical statuses for this fraction',
    })
    @OneToMany(
        () => FractionStatus,
    ...
    @ApiPropertyOptional({
        type: () => Incident,
        isArray: true,
        description: 'Incidents registered against this fraction',
    })
    @OneToMany(() => Incident, (incident) => incident.fraction, {
        cascade: false,
        eager: false,
        onDelete: 'NO ACTION',
    })
    ...
    updateInsert(): void {
        ...
        const [hours, minutes, seconds] = this.timeByBlock
            .split(':')
            .map(Number);
        ...
        // Adjust the new time
        const [adjustedHours, adjustedMinutes, adjustedSeconds] =
            this._adjustTime([newHours, newMinutes, newSeconds]);
        ...
        if (this.beforeTime) {
            const [beforeHours, beforeMinutes, beforeSeconds] =
                this.beforeTime.split(':');
        ...
        if (this.registerAt && this.time) {
            ...
            registerDateTime.setHours(
                registerDateTime.getHours() + Number(hours),
            );
            registerDateTime.setMinutes(
                registerDateTime.getMinutes() + Number(minutes),
            );
            registerDateTime.setSeconds(
                registerDateTime.getSeconds() + Number(seconds),
            );
