...
export class FractionService {
    ...
    async findFractionHistory(filterDto: FilterDto) {
        ...
        try {
            ...
            if (year && month) {
                ...
                tableExistsFraction =
                    await this._tableExists(tableNameForCheck);
    ...
    async findAllTotalOccupationRotationParking(filterDto: FilterDto) {
        ...
        `;
                ...
                if (fractions.length > 0) {
                    ...
                    fractions[0].rotation = (
                        +fractions[0].totalParking / +total
                    ).toFixed(2);
                }
                return { fractions };
            } else {
    ...
    async findStatisticsFractions(filterDto: FilterDto) {
        try {
            ...
            if (dateFrom && dateTo) {
                const fromSource =
                    await this._buildStatisticsFractionRangeSource(
                        dateFrom,
                        dateTo,
                    );
                if (!fromSource) {
                ...
                return await this._runStatisticsFractionQuery(
                    fromSource,
                    filterDto,
                );
            }
    ...
    private async _runStatisticsFractionQuery(
        ...
        const { parameters, conditions } =
            this._buildQueryParameters(filterDto);
        const query = this._buildStatisticsFractionQuery(
            fromSource,
            conditions,
        );
        const fractions = await this.fractionRepository.query(
