...
export class Status {
  @PrimaryColumn({
    ...
    comment: 'Manual primary key matching the StatusFraction enum values',
  })
  @IsPositive()
  id: number;
  ...
  @Column('varchar', {
    length: 40,
    comment: 'Human-readable name for this fraction status',
  })
  name: string;
  ...
  @OneToMany(() => Fraction, (fraction) => fraction.status, {
    cascade: false,
    eager: false,
    onDelete: 'NO ACTION',
  })
  ...
  @OneToMany(() => FractionStatus, (fractionStatus) => fractionStatus.status, {
    cascade: false,
    eager: false,
    onDelete: 'NO ACTION',
  })
